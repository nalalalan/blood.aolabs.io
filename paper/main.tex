...
The main design choice is transparency. The score can be computed by hand from the table. The front page shows the score first because that is the current state Alan asked to see, and it places the graphs immediately after the score because the graphs are the source audit. The same structure also keeps recommendations bounded. Blood selects the next suggestion from the strongest positive factor, not from a generated personality profile. The wording is deliberately simple but not vague: current time block, source reason, and one food, water, or movement adjustment. If low glucose is the strongest factor, Blood names the glucose value and asks for carb plus protein now more and hard exercise less. If heart rate is strongest, it names the bpm value and asks for water plus light movement more and hard exercise less, or water plus easy walking more and sitting still less. If estimated HRV is strongest, it names the estimated HRV value and asks for water plus small food more and sitting still less. If sleep is short, it names the sleep duration and asks for water plus simple food more and hard exercise less. If steps are low, it names the recent step count and asks for a short walk more and sitting still less. The time label is not a delay instruction or learned pattern; it is only the current Eastern time block: morning, midday, afternoon, evening, or night.
